...
def evaluate(thr, settle, pad):
    sim.MOTION_THRESHOLD, sim.SETTLE_FRAMES = thr, settle
    mats, dropped = [], 0
    for i in range(n):
...
print(f"{'umbral':>7} {'settle':>7} {'pad':>4}  top1   top3   perdidos")
for thr in (0.08, 0.05, 0.03, 0.02):
    for settle in (6, 10):
        for pad in (0, 6):
            t1, t3, d = evaluate(thr, settle, pad)
            print(f"{thr:>7} {settle:>7} {pad:>4}  {t1:.3f}  {t3:.3f}   {d}")
